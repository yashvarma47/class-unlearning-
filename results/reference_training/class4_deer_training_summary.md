# W_ref for class 4 (deer)

Retain-only reference: trained on `D_r_train` only, the 45,000 CIFAR-10 training images that are **not** deer. It never saw a single deer.
...
|---|---|
| epoch | 189 |
| `D_r_test` accuracy | 0.9457 |
| `D_r_test` loss | 0.2203 |
| `D_f_test` accuracy | 0.0000 (diagnostic only -- near zero is CORRECT) |
| `D_f_test` loss | 9.3447 |
| full test accuracy | 0.8511 (diluted by the held-out class; not a utility number) |

File: `class4_deer_reference_best_dr.pt`

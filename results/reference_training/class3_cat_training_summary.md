# W_ref for class 3 (cat)

Retain-only reference: trained on `D_r_train` only, the 45,000 CIFAR-10 training images that are **not** cat. It never saw a single cat.
...
|---|---|
| epoch | 195 |
| `D_r_test` accuracy | 0.9612 |
| `D_r_test` loss | 0.1530 |
| `D_f_test` accuracy | 0.0000 (diagnostic only -- near zero is CORRECT) |
| `D_f_test` loss | 10.4254 |
| full test accuracy | 0.8651 (diluted by the held-out class; not a utility number) |

File: `class3_cat_reference_best_dr.pt`

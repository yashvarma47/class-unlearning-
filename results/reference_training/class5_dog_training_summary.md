# W_ref for class 5 (dog)

Retain-only reference: trained on `D_r_train` only, the 45,000 CIFAR-10 training images that are **not** dog. It never saw a single dog.
...
|---|---|
| epoch | 151 |
| `D_r_test` accuracy | 0.9576 |
| `D_r_test` loss | 0.1664 |
| `D_f_test` accuracy | 0.0000 (diagnostic only -- near zero is CORRECT) |
| `D_f_test` loss | 11.0671 |
| full test accuracy | 0.8618 (diluted by the held-out class; not a utility number) |

File: `class5_dog_reference_best_dr.pt`
